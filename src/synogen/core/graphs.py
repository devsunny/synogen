...
from synogen.core.agents import Agent, START, END
from synogen.core.tools import function_to_openai_tool
...
class AgentGraph:
    ...
    def get_adjacent_nodes(self, node_name: str | Node) -> list[Node]:
        """
        ...
        node_id = None
        node_name = node_name.name if isinstance(node_name, Node) else node_name
    ...
    def _get_tools_metadata(self, nodes: List[Node]) -> List[Dict[str, Any]]:
        """Extract OpenAI tool metadata from a list of nodes."""
        tools = []
        for n in nodes:
            meta = n.runner.metadata()
            if meta:
                tools.append(meta)
        return tools
    ...
    def execute(
        ...
        if len(start_nodes) == 1 and start_nodes[0].name == START:
            # Inject initial messages to the single StartAgent node
            start_nodes = self.get_adjacent_nodes(START)
